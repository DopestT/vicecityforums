...
- Gives admins a command center at `/vicewire.html`, including a secure password-reset entry point.
...
The workflow is intentionally manual-only when first deployed. After the owner has verified the command center, database policies, and a successful manual run, add an hourly schedule deliberately.
